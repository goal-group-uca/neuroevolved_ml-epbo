...
from getSolutionDetails import get_solution_details, get_solution_details2
...
def create_dataset(config_input_route):
    ...
    for route in tqdm.tqdm(routes):
        for scenario in scenarios:
            ...
            if len(splitted) == 2:
                speed_route = splitted[0]
                value = splitted[1].replace("speed", "").replace("0", "")
                speed = f"_speed_0.{value}"
            data = pd.read_csv("../route_info/processed_bus_route_{}_random_{}%ze{}.csv".format(speed_route.replace("Bus",""), float(scenario.replace("%", "")), speed), index_col=0)
            print("../route_info/processed_bus_route_{}_random_{}%ze{}.csv".format(speed_route.replace("Bus",""), float(scenario.replace("%", "")), speed))
            print("{}/{}_Experiment/Hybrid_Bus{}_individuals.pf".format(config_input_route, route, scenario))
            if os.path.exists("{}/{}_Experiment/Hybrid_Bus{}_individuals.pf".format(config_input_route, route, scenario)):
                with open("{}/{}_Experiment/Hybrid_Bus{}_individuals.pf".format(config_input_route, route, scenario), "r") as filename:
                    for line in filename.readlines():
                        ...
                        invalid , soc_values, remaining_charges, charge_per_zone, emissions_per_zone, green_kms_per_zone, solution = get_solution_details2(solution,
                        "../route_info/processed_bus_route_{}_random_{}%ze{}.csv".format(speed_route.replace("Bus",""), float(scenario.replace("%", "")), speed))
                        if not invalid:
                            solution_index = 0
...
def get_context(id, n_before, n_after):
    ...
    if "|" in path:
        path = path.split('|')
        speed = f"_speed_{path[2].split('_')[0]}"
        path = path[0] + '_' + path[2].split('_')[-1]

    path = path.split('_')
    percent = float(path[-1].replace("%", ""))
    label = path[1]

    data = pd.read_csv("../route_info/processed_bus_route_{}_random_{}%ze{}.csv".format(label.replace("Bus",""), percent, speed), index_col=0)
